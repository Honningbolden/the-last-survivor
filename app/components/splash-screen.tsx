...
export default function SplashScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className={'flex flex-col h-screen justify-center items-center bg-white text-black'}>
      ...
      <button className='px-6 py-3 font-semibold text-base bg-green-300 rounded-lg my-12' onClick={toggleOverlay}>
        Start Experience 🚀
      </button>
      {overlay && (
        <div className={'absolute h-screen w-screen flex flex-col items-center justify-center p-12 border rounded-3xl bg-white'}>
          ...
          <p className='text-sm font-normal text-pretty max-w-screen-md text-center mb-8'>
            ...
            <br />
            You can either use your keyboard and mouse, or you can use the orientation of your hands and distance between your fingers (requires webcam)
          </p>
          <div className={'flex flex-row gap-12 justify-center gap-4 w-full m-6'}>
            <span className='flex flex-col items-center text-xs group'>
              <button className='px-6 py-3 font-semibold text-base bg-gray-300 text-gray-600 rounded-lg' disabled onClick={() => onStart(false)}>
                🎮 Keyboard & Cursor
              </button>
              <span className='absolute -bottom-4 opacity-0 group-hover:opacity-100'>Still button doesn't work yet!</span>
            </span>
            <button className='px-6 py-3 font-semibold text-base bg-green-300 rounded-lg' onClick={() => onStart(true)}>
              🤙 Hands & Webcam
            </button>
          </div>
        </div>
